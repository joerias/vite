...
const { bookObj: b } = require("./utils.cjs");
const {
	convertPropValue,
...
} = require("./logic-helper.cjs");
const { varTestReg } = require("./regexs.cjs");
const fs = require("fs");
const path = require("path");

function appendDemoContent(
	postcss,
	selector,
	rule,
	desktopViewAtRule,
...
function appendConvertedFixedContainingBlockDecls(
	postcss,
	selector,
	decl,
	disableDesktop,
		...
		disableMobile,
		{
			...
			convertMobile: (number, unit, numberStr) => {
				if (isFixed) {
					if (leftOrRight) {
						if (limitedWidth) {
							return convertMaxMobile_FIXED_LR(
								number,
								unit,
								maxDisplayWidth,
								viewportWidth,
...
function appendDisplaysRule(
	...
	selector,
	postcss,
	{ sharedAtRule, desktopViewAtRule, landScapeViewAtRule, isShare }
) {
...
function appendMediaRadioPxOrReplaceMobileVwFromPx(
	postcss,
	selector,
	prop,
	val,
...
function appendCSSVar(
	...
	selector,
	postcss,
	{ sharedAtRule, desktopViewAtRule, landScapeViewAtRule, isLastProp }
) {
...
function appendMarginCentreRootClassNoBorder(
	postcss,
	selector,
	disableDesktop,
	disableLandscape,
	{ desktopViewAtRule, landScapeViewAtRule, sharedAtRule, desktopWidth, landscapeWidth }
) {
...
function appendMarginCentreRootClassWithBorder(
	postcss,
	selector,
	disableDesktop,
	disableLandscape,
	{ desktopViewAtRule, landScapeViewAtRule, sharedAtRule, dvhAtRule, desktopWidth, landscapeWidth, borderColor }
) {
	if (!disableDesktop || !disableLandscape) {
		if (dvhAtRule.nodes.length === 0) dvhAtRule.append(postcss.rule({ selector }).append(minDFullHeight));
	}
	if (disableDesktop && !disableLandscape) {
		// 仅移动端横屏
		landScapeViewAtRule.append(
		...
		desktopViewAtRule.append(
			postcss
				.rule({ selector })
				.append(maxWidth(desktopWidth), marginL, marginR, shadowBorder(borderColor), minFullHeight, autoHeight)
		);
	} else if (!disableDesktop && !disableLandscape) {
		// 桌面和移动端横屏
		desktopViewAtRule.append(postcss.rule({ selector }).append(maxWidth(desktopWidth)));
		landScapeViewAtRule.append(postcss.rule({ selector }).append(maxWidth(landscapeWidth)));
		sharedAtRule.append(
			postcss.rule({ selector }).append(marginL, marginR, shadowBorder(borderColor), minFullHeight, autoHeight)
		);
	}
}

function appendCentreRoot(
	postcss,
	selector,
	disableDesktop,
	disableLandscape,
...
function appendSiders(postcss, siders, desktopWidth, maxLandscapeDisplayHeight) {
	...
	const sideRule1 =
		siders[0].selector != null
			? postcss
					.rule({ selector: siders[0].selector })
					.append(
						fixedPos,
						top(siders[0].gap),
						sideL(desktopWidth, siders[0].gap, side1W),
						autoDir("right"),
						autoDir("bottom"),
						width(side1W)
					)
			: null;
	const sideRule2 =
		siders[1].selector != null
			? postcss
					.rule({ selector: siders[1].selector })
					.append(
						fixedPos,
						top(siders[1].gap),
						autoDir("left"),
						sideR(desktopWidth, siders[1].gap, side2W),
						autoDir("bottom"),
						width(side2W)
					)
			: null;
	const sideRule3 =
		siders[2].selector != null
			? postcss
					.rule({ selector: siders[2].selector })
					.append(
						fixedPos,
						autoDir("top"),
						autoDir("left"),
						sideR(desktopWidth, siders[2].gap, side3W),
						bottom(siders[2].gap),
						width(side3W)
					)
			: null;
	const sideRule4 =
		siders[3].selector != null
			? postcss
					.rule({ selector: siders[3].selector })
					.append(
						fixedPos,
						autoDir("top"),
						sideL(desktopWidth, siders[3].gap, side4W),
						autoDir("right"),
						bottom(siders[3].gap),
						width(side4W)
					)
	...
	const atRules = sidersRule.reduce((acc, [side, rule]) => {
		...
		const atRule = postcss
			.atRule({
				name: "media",
				params: `(min-width: ${
					desktopWidth + sideW * 2 + side.gap * 2
				}px) and (min-height: ${maxLandscapeDisplayHeight}px)`,
				nodes: [],
			})
			.append(rule);
