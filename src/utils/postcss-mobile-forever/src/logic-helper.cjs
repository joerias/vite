const { unitContentMatchReg, fixedUnitContentReg } = require("./regexs.cjs");
const {
	containingBlockWidthProps,
	horisontalContainingBlockLogicalProps,
	verticleContainingBlockLogicalProps,
...
const createExcludeFunc = (TYPE_REG, TYPE_ARY) => (exclude, file, regOrAry) => {
	if (exclude && file) {
		...
		if (regOrAry === TYPE_ARY) {
			for (let i = 0; i < exclude.length; ++i) if (exclude[i].test(file)) return true;
		}
	}
};
	...
	do {
		if (next && next.type === "comment" && next.text === comment) {
			if (/\n/.test(next.raws.before)) {
				result.warn("Unexpected comment /* " + comment + " */ must be after declaration at same line.", {
					node: next,
				});
			} else {
				// remove comment
	...
	if (isFixed) {
		if (leftOrRight) {
			if (unit === "px") {
				...
				if (number === 0) return noUnitZeroToMediaQueryPx_FIXED_LR(idealWidth);
				return `${number}${unit}`;
...
const convertNoFixedMediaQuery = (number, idealWidth, viewportWidth, precision, unit, numberStr) => {
	if (unit === "vw") return vwToMediaQueryPx(number, idealWidth, precision, numberStr);
	else if (unit === "px") {
		return pxToMediaQueryPx(number, viewportWidth, idealWidth, precision, numberStr);
	} else return `${number}${unit}`;
};
...
const convertMaxMobile_FIXED_LR = (number, unit, maxDisplayWidth, viewportWidth, unitPrecision, numberStr) => {
	if (unit === "px") return pxToMaxViewUnit_FIXED_LR(number, maxDisplayWidth, viewportWidth, unitPrecision);
	else if (unit === "vw") return vwToMaxViewUnit_FIXED_LR(number, maxDisplayWidth, unitPrecision);
	else if (unit === "%") return percentToMaxViewUnit_FIXED_LR(number, maxDisplayWidth, unitPrecision);
	else if (unit === " " || unit === "") {
		if (number === 0) return `calc(50% - min(50%, ${maxDisplayWidth / 2}px))`;
		return `${number}${unit}`;
	} else return `${numberStr}${unit}`;
};

/** 转换移动端竖屏，包含块是根元素 */
const convertMaxMobile_FIXED = (
	number,
	unit,
	maxDisplayWidth,
	viewportWidth,
