<template>
	...
	>
		...
		>
			...
			/>
			<ImageUpload v-if="isUniFrameWork" imageSourceType="camera" />
			<ImageUpload imageSourceType="album" />
			<FileUpload v-if="!isUniFrameWork" />
			<VideoUpload videoSourceType="album" />
			<VideoUpload v-if="isUniFrameWork" videoSourceType="camera" />
			<Evaluate />
			<Words />
...
const extensionListShowInStart = computed((): ExtensionInfo[] =>
	isPC
		? currentExtensionList.value.filter((extension: ExtensionInfo) => extension?.data?.name !== "search")
		: currentExtensionList.value
);
